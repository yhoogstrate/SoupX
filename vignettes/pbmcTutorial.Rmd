...
### Droplets used for the Soup
To visualise the 
```{r}
data = system.file(package = "SoupX", "extdata/toyData")
sc = load10X(data, keepDroplets=TRUE)
plotDropletResolution(sc) +
  ggplot2::labs(subtitle="UMIs per droplet in: SoupX toyData")
sc = removeNoncorrectedDroplets(sc) # cleanup
```

To preserve all Droplets, you can load using `keepDroplets=TRUE`, which can be useful for QC or for optimizing the thresholds for Droplets used to build the SoupChannel. A cleanup function can be used afterwards. The thresholds can be adjusted as follows:

```{r}
sc = load10X(data, soupRange=c(55,70), keepDroplets=TRUE)
plotDropletResolution(sc) +
  ggplot2::labs(subtitle="UMIs per droplet in: SoupX toyData")
sc = removeNoncorrectedDroplets(sc) # cleanup
```

### Sample spike-in Droplets from the Soup
...
### Create SoupChannel with Seurat objects
